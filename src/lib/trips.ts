...
export async function getTrips(): Promise<Trip[]> {
  ...
  if (!memberTripIds.length) return ownedTrips
  ...
  for (const trip of [...ownedTrips, ...(sharedTrips ?? [])]) uniqueTrips.set(trip.id, trip)
...
export async function deleteTrip(id: string) {
  const { data: { user }, error: userError } = await supabase.auth.getUser()
  if (userError) throw userError
  if (!user) throw new Error('You must be signed in to delete a trip')
  ...
  if (error) throw error
  if (!data?.length) throw new Error('Only the trip owner can delete this trip')
}
...
export async function reActivateTrip(id: string) {
  const { error } = await supabase.from('trips').update({ status: 'active', cancelled_at: null, cancelled_by: null }).eq('id', id)
  if (error) throw error
}
...
export async function getTripInvitations(tripId: string): Promise<TripInvitation[]> {
  const { data, error } = await supabase.from('trip_invitations').select('*').eq('trip_id', tripId).order('created_at', { ascending: false })
  if (error) throw error
  return (data ?? []) as TripInvitation[]
}

export async function inviteTripMember(tripId: string, email: string) {
  const { data, error } = await supabase.functions.invoke('invite-trip-member', { body: { tripId, email } })
  if (error) throw error
  return data
}

export async function acceptTripInvitation(token: string) {
  const { data, error } = await supabase.functions.invoke('accept-trip-invite', { body: { token } })
  if (error) throw error
  return data
}
